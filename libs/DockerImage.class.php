...
class DockerImage{
	...
	public function imgTag($imgIdArr,$newRepo,$newTag){
		// foreach ($imgIdArr as $imgId) {
			$this->cmObj->curlPost($this->url.$imgIdArr[0].'/tag?repo='.$newRepo.'&force=0&tag='.$newTag);
		// }
	...
	public function imgInspect($imgIdArr){
		// foreach ($imgIdArr as $imgId) {
			return $this->cmObj->curlGet($this->url.$imgIdArr[0].'/json');
		// }
	}

	public function imgHistory($imgIdArr){
		// foreach ($imgIdArr as $imgId) {
			$history=$this->cmObj->curlGet($this->url.$imgIdArr[0].'/history');
